...
By default the setting for `wireguard_allowed_ips` is `AllowedIPs = {{hostvars[host].wireguard_ip}}/32` (have a look `templates/wg.conf.j2`). Let's see this example and let's assume you don't set `wireguard_allowed_ips` explicitly:
...
Now this is basically the same as above BUT now the config says: I want to route EVERY traffic originating from my workstation to the endpoint `controller01.p.domain.tld:51820`. If that endpoint can handle the traffic is of course another thing and is up to you and how you configure the endpoint routing ;-)
...
The other WireGuard config files (`wg0.conf` by default) looks similar but of course `[Interface]` includese the config of that specific host and the `[Peer]` entries lists the config of the other hosts.
